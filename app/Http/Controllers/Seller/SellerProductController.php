...
class SellerProductController extends Controller
{
    ...
    {
       $datas=Product::where(['seller_id'=>Auth::user()->id,'product_type'=>'seller'])->orderBy('id','DESC')->get();
        ...
        ->addIndexColumn()
         ...
         ->editColumn('attribute',function(Product $data){
                 return '<div class="btn-group btn-group-medium" role="group" aria-label="Basic example">
                              ...
                              <a href="javascript:void(0)" data-target="#myModal" data-toggle="modal" class="btn btn-dark btn-icon show_product_status" data-action="'.route('seller.show_product_status').'" product_id="'.$data->id.'">
                                  <span class="btn-icon-label"><i class="mdi mdi-check-all mr-2"></i></span> Status
                              </a>
                              <a href="'.route('seller.product_detail',$data->slug).'" class="btn btn-dark btn-icon">
        ...
        ->editColumn('action',function(Product $data){
                 return '<a href="'.route('seller.product_edit',$data->slug).'" class="btn btn-success btn-sm">
      ...
      if ($request->isMethod('post'))
        {
            ...
            try{
                ...
                if($request->hasFile('thumbnail')){
                        ...
                        Image::make($image)->resize(1100,1100)->save($medium_image_path);
  ...
  public function edit($slug)
  {
   ...
   public function product_detail($slug)
   {
     ...
     $product=Product::where('slug',$slug)->first();
     return view('seller.product.detail',compact("product"));
   }
